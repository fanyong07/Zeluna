...
- Current HEAD: latest completed implementation slice `d7518d2` (use `git rev-parse HEAD` for the progress-only commit that follows)
...
- `d7518d2 test: create release fixture in clean CI`

- Quality Gates #127 exposed that the new immutability regression assumed an
  existing ignored `release/` directory on a clean Windows runner. The test
  fixture now creates that directory when absent and removes it only when the
  test created it and it remains empty.
- GitHub Actions Quality Gates run `31255314053` (#128) completed successfully
  on `d7518d2`: Flutter analyze/test, Android Debug, Windows/WebView (including
  release immutability), Web, Python/Alembic/Ruff/pip-audit, and supply-chain
  jobs all passed.

### Acceptance boundaries

- The owner explicitly selected the current internal Release APK scope and the
  historical compatibility certificate; a store release, production signing
  identity, and AAB are not required for this Goal's final acceptance.
- The selected identity remains an Android Debug certificate. It is accepted
  only for internal overwrite-install continuity and must not be represented
  as a secure public/store identity. `tool/check_release.ps1` correctly keeps
  public-release blockers active; no safety check was disabled or weakened.
...
G14 is complete for the owner-selected internal Release scope: G0-G14 code,
migrations, policies, cross-platform builds, local/full test gates, MuMu signed
overwrite installation, device network security, artifact immutability, and
the latest six-job CI run are verified. Public/store distribution remains a
separate future release decision rather than an unverified completion claim.
