...
namespace flutter_inappwebview_plugin
{
    ...
    ChannelDelegate(plugin->registrar->messenger(), HeadlessInAppWebViewManager::METHOD_CHANNEL_NAME)
  {
    ...
    windowClass_.lpfnWndProc = &HeadlessInAppWebViewManager::CleanupWindowProc;

    RegisterClass(&windowClass_);
    cleanupWindow_ = CreateWindowEx(0, windowClass_.lpszClassName, L"", 0,
      0, 0, 0, 0, HWND_MESSAGE, nullptr, windowClass_.hInstance, this);
  }

  LRESULT CALLBACK HeadlessInAppWebViewManager::CleanupWindowProc(HWND window, UINT message, WPARAM wparam, LPARAM lparam)
  {
    ...
    const auto manager = reinterpret_cast<HeadlessInAppWebViewManager*>(
      GetWindowLongPtr(window, GWLP_USERDATA));
    if (message == WM_DISPOSE_HEADLESS_WEBVIEW && manager) {
      manager->drainPendingDisposals();
      return 0;
    }
  ...
  void HeadlessInAppWebViewManager::scheduleDispose(const std::string& id)
  {
    if (!cleanupWindow_) {
      return;
    }
    pendingDisposals_.push_back(id);
    PostMessage(cleanupWindow_, WM_DISPOSE_HEADLESS_WEBVIEW, 0, 0);
  }

  void HeadlessInAppWebViewManager::drainPendingDisposals()
  {
    std::vector<std::string> disposals;
    disposals.swap(pendingDisposals_);
    for (const auto& id : disposals) {
      const auto webView = webViews.find(id);
      if (webView != webViews.end()) {
        webViews.erase(webView);
      }
    }
  }
  ...
  HeadlessInAppWebViewManager::~HeadlessInAppWebViewManager()
  {
    ...
    pendingDisposals_.clear();
    webViews.clear();
    if (cleanupWindow_) {
      DestroyWindow(cleanupWindow_);
      cleanupWindow_ = nullptr;
    }
