...
- `ANICH_*`：番剧聚合源 `crawler.anich` 的按需代取参数。该源必须在 `PLAYBACK_PROVIDER_IDS` 中显式点名才会启用（灰度示例：`PLAYBACK_PROVIDER_IDS=aggregate.maccms,crawler.anich`），去掉该 ID 即完成回滚。
  ...
  - `ANICH_MAX_LINES_PER_EPISODE`：默认 `6`，目前仅保留给诊断表达优先验证预算；不是展示/返回上限，适配器仍返回全部合法的不同本集地址，不得用它截断现有线路。
  - `PLAYBACK_ANICH_LINE_TTL_HOURS`：该源直链无签名参数但按天易腐，默认对其单独盖 `2` 小时逐线过期；设为 `0` 关闭盖章，改由整行缓存 TTL 兜底。

## 增量原站直连（保留现有线路）

- `crawler.xifan`：稀饭原站独立搜索、详情和取流，本地代码已注册；不调用 AniCh。2026-09-13 已完成当前 Windows 网络的真实作品自动匹配、媒体读取及首帧样本检查，生产出口和客户端尚未验证。
- 部署需要独立授权。启用时向**现有** `PLAYBACK_PROVIDER_IDS` 追加 `crawler.xifan`，不得照抄一个短列表覆盖原配置，更不得因新增直连移除 `crawler.anich` 或其他现有 ID。
- 该站搜索串行且在上次响应后至少等待 4.2 秒，成功搜索元数据暂存 60 秒；不保存播放地址，不通过换入口绕过验证。暂时失败不删除线路配置。
- 上线验收应比较同作品、同集的旧/新清单；相同媒体经两种取流方式取得，不算两个独立片源。未知原站的旧线路保留兜底，不把 HTTP 200 或候选数量当作播放成功。
- 回滚仅撤销本次新增 ID，保留原有列表。详见 `docs/research/independent-anime-source-mapping.md` 与 `tests/test_source_migration.py`。


### 综合影视接入补齐（2026-09-13，本地开发）

- 现有 MacCMS 来源本来直接访问各资源站 API，不经过 AniCh。正式站表 17 个启用来源与 3 个原隔离条目全部保留；爱奇艺、量子补齐电视剧查询，电影天堂补齐电影查询，不改变其分层、quick 或 precache。
- 电影子分类和剧集分类、跨播放组集数合并、同站多线路身份以及媒体 Referer/Origin 透传已补齐；后端仍返回稳定作品和分集身份，不拿预告/解说或不同季数替代正片。
- 非凡资源、新浪资源、U酷资源在 `data/maccms_candidates.json` 中更新为站方当前 API，原始发现来源和候选状态保留，候选数量仍为 80。已用隔离环境验证三类适配链路并抽查真实影视样本；**没有加入正式站表，启用 aggregate.maccms 不会自动启用它们**。
- 非凡核验到的公开元数据 API 为 HTTP，不传账号凭据，但存在传输风险；目标出口、内容/条款和网络策略需要按下文 smoke/promotion 与人工审核流程确认。本机首帧成功不等于生产审核通过。
- 闪电、天涯不支持公开标题搜索的现状和其他未确定候选均保留，不以不可搜索或单次超时删减库存。本轮未部署、未改生产环境、未打包，详细证据和剩余边界见 `docs/research/general-video-origin-candidates-2026-09-13.md`。

### 来源内容范围规则（批注修复，本地代码已落地）

- **不得因为适配器位于 `scrapers/anime/`、站名带“动漫”或最初只接番剧，就在搜索前排除电影、电视剧。** 通用搜索和解析器具备相应能力时，`content_types` 应声明可查询的板块；它是技术能力，不是对源站当前库存的保证。源站有哪一类，以单个条目的分类/详情证据决定。
- 稀饭、西瓜卡通、GiriGiri、樱花 yhdmm，以及共用的配置化 HTML 适配器支持三类查询。动画电影/剧场版按 `movie`，明确电视剧/特摄剧按 `tv`（provider 能力名为 `series`）；动画属性与电影/剧集形式分开保留。
- 分类只取当前搜索卡片、当前详情的类别/年份/明确版本标记。不得扫描整个导航、推荐卡片或简介来猜类型；单集数量、站点名称、CDN 域名不是电影/真人剧证据。站点默认 `anime` 仅作旧番剧兼容，不能把它升级为电影/真人剧匹配。
- 搜索提示和旧本地索引缺分类时，仅给标题相关候选补详情：每个来源、每轮发现跨所有别名最多 3 个详情，单个最多 2 秒并发读取；复用结果并继续经过既有年份、季数、衍生版本校验。失败不删源，不把未知分类当正片证据，不为扩大线路数量放宽明确身份冲突。
- 樱花本地索引跟随首页真实同源分类路径，补入电影/剧集目录，而不是只抓旧数字列表。最多额外 12 个分类，不接外域/查询串；仍保留旧列表。前台惰性构建限 5.5 秒，保留已收集结果，临时索引不冒充完整定期索引。
- 明确电影条目中的 HD、1080P、4K、国语等完整版本归同一集的备用线路，不把数字当集号；同站已有播放组/标签保留。普通连续剧按原来的集号和季数规则处理，不把上/下部、分段或合集盲目合并。
- 新增/修改来源要覆盖：实际搜索→详情→对应板块发现、缺分类补证、同名动画/真人与电影/剧集冲突、年份/季数冲突、电影版本和多播放组保留。正向控制测试不等于源站真实有该库存；源站确实提供的类别再做真实作品和媒体验收。
- 内容范围规则本身不改变 provider 启用状态、不替换 AniCh 兜底、不晋级未经审核的候选。授权上线时只在现有 allowlist 追加本轮出口验证通过的独立源；不得照抄短列表覆盖已有来源。发布与验线边界见对应版本说明。
...
在隔离候选代码目录、使用目标 VPS 出口执行：

```bash
python tools/probe_all_routes.py --output /安全的证据目录/routes.json --egress production-vps --include-candidates
```

- 默认覆盖正式20条、全部13个爬虫和5个旧TVBox兼容接口；选项补上80条候选。
- 真人剧、真人电影、番剧和动画电影四种真实作品样本；剧集检查第1/3集。所有返回线路都检查，不因为先前成功就提前结束。
